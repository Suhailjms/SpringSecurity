...
public class JWTService {
    ...
    private SecretKey getKey() {
        byte[] keyBytes = Decoders.BASE64.decode(secretkey); // which converts the string to keybytes
        return Keys.hmacShaKeyFor(keyBytes); // returning the key with hmacsh
    }

    public String extractUserName(String token) {
        // extract the username from jwt token
        // username is the part of claim
    ...
    private Claims extractAllClaims(String token) {
        return Jwts.parser()
                .verifyWith(getKey()) // works with secret key which above
                .build()
